...
class EvalContextProvider:
    ...
    def __init__(self, custom_slides: list[dict[str, Any]] | None = None):
        if custom_slides:
            self.repo = (
                LocalSlideRepository(slides=custom_slides)
                if LocalSlideRepository
                else None
            )
    ...
    def get_context(
        ...
        if (
            context_fixture
            and context_fixture.type == "synthetic_slides"
            and context_fixture.slides
        ):
            slide_dicts = [s.model_dump() for s in context_fixture.slides]
            repo = (
                LocalSlideRepository(slides=slide_dicts)
                if LocalSlideRepository
                else None
            )
            if repo:
