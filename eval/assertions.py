...
def evaluate_turn_assertions(
        ...
        for pair in expected.expected_tool_order:
            if len(pair) == 2:
                ...
                if not t1_in or not t2_in:
                    ...
                    order_msgs.append(
                        f"Pair [{t1}, {t2}] failed: missing tool(s) {missing}"
                    )
                else:
                    ...
                    if idx1 >= idx2:
                        order_passed = False
                        order_msgs.append(
                            f"'{t1}' (index {idx1}) came after '{t2}' (index {idx2})"
                        )
    ...
    if expected.assistant_message_required is True:
        has_msg = bool(
            turn_res.assistant_message and turn_res.assistant_message.strip()
        )
        msg = (
            "Assistant message present"
            if has_msg
            else "Assistant message is missing or empty"
        )
    ...
    if expected.grounding_required is True:
        ...
        status_ok = turn_res.grounding_valid is True
        valid_sources = True
        if turn_res.retrieved_sources and turn_res.citation_ids:
            # Citation IDs must exactly belong to current retrieved source IDs.
    ...
    if expected.required_citation_ids:
        required = set(expected.required_citation_ids)
        passed = required.issubset(set(turn_res.citation_ids))
        _add(
            "required_citation_ids",
            passed,
            f"Required citation IDs {required}, got {turn_res.citation_ids}",
            "citation",
        )

    if expected.forbidden_citation_ids:
        forbidden = set(expected.forbidden_citation_ids).intersection(
            turn_res.citation_ids
        )
        _add(
            "forbidden_citation_ids",
            not forbidden,
            f"Forbidden citation IDs present: {sorted(forbidden)}",
            "citation",
        )
    ...
    if expected.required_retrieval_order_prefix:
        expected_prefix = expected.required_retrieval_order_prefix
        actual_prefix = turn_res.retrieved_sources[: len(expected_prefix)]
        _add(
            "required_retrieval_order_prefix",
            actual_prefix == expected_prefix,
            f"Expected retrieval prefix {expected_prefix}, got {actual_prefix}",
            "retrieval",
        )
    ...
    if expected.required_deck_ids or expected.forbidden_deck_ids:
        ...
        if expected.forbidden_deck_ids:
            intersection = set(expected.forbidden_deck_ids).intersection(
                retrieved_decks
            )
            passed = len(intersection) == 0
    ...
    if expected.followup_schema_required is True and num_followups > 0:
        ...
        msg = (
            "Followup schema valid"
            if valid_schema
            else "Followups missing label or question"
        )
        _add("followup_schema_required", valid_schema, msg, "followup")

    if expected.followups_unique is True and num_followups > 0:
        q_texts = [
            str(f.get("question", "")).strip().lower() for f in turn_res.followups
        ]
        unique_passed = len(q_texts) == len(set(q_texts))
        msg = (
            "All followups are unique"
            if unique_passed
            else "Duplicate followup questions detected"
        )
    ...
    if expected.followups_not_duplicate_query is True and num_followups > 0:
        ...
        no_dup_q = not any(
            _string_similarity(str(f.get("question", "")), user_q) > 0.85
            for f in turn_res.followups
        )
        msg = (
            "No followup duplicates user query"
            if no_dup_q
            else "Followup question duplicates user query"
        )
    ...
    if expected.followups_not_duplicate_answer is True and num_followups > 0:
        ...
        no_dup_ans = not any(
            _string_similarity(str(f.get("question", "")), ans_text) > 0.85
            for f in turn_res.followups
        )
        msg = (
            "No followup duplicates assistant answer"
            if no_dup_ans
            else "Followup question duplicates assistant answer"
        )
        _add("followups_not_duplicate_answer", no_dup_ans, msg, "followup")

    if (
        expected.generic_followups_forbidden is True
        or expected.min_followups is not None
    ) and num_followups > 0:
        generic_found = False
        for f in turn_res.followups:
            ...
            if any(pattern in q_text for pattern in GENERIC_FOLLOWUP_PATTERNS):
                ...
                break
        msg = (
            "No generic followups detected"
            if not generic_found
            else "Generic followup pattern detected"
        )
    ...
    if expected.no_stale_citations is True:
        ...
        if turn_res.citation_ids:
            ...
            for cid in turn_res.citation_ids:
                if retrieved_set and cid not in retrieved_set:
                    stale_passed = False
            ...
            if stale_passed and previous_turn_res and previous_turn_res.citation_ids:
                ...
                if (
                    prev_cit_set == curr_cit_set
                    and turn_res.input_text != previous_turn_res.input_text
                    and not turn_res.retrieved_sources
                ):
                    stale_passed = False
    ...
    if expected.new_check_required is True:
        has_check = turn_res.status == "awaiting_check" and bool(
            turn_res.check_question or turn_res.check_id
        )
        is_distinct = True
        if has_check and previous_turn_res and previous_turn_res.check_question:
            sim = _string_similarity(
                turn_res.check_question or "", previous_turn_res.check_question or ""
            )
            if sim > 0.85 or (
                turn_res.check_id and turn_res.check_id == previous_turn_res.check_id
            ):
                is_distinct = False
    ...
    if expected.no_duplicate_action is True and previous_turn_res:
        ...
        is_dup = bool(
            prev_act
            and curr_act
            and prev_act == curr_act
            and turn_res.status == previous_turn_res.status
        )
        msg = (
            "No duplicate action"
            if not is_dup
            else f"Duplicate action ID '{curr_act}' recreated"
        )
    ...
    if expected.expected_failure_code:
        ...
        passed = (
            expected.expected_failure_code == turn_res.failure_code
            or expected.expected_failure_code in err_msg
        )
    ...
    if expected.expected_blocked is True:
        ...
        msg = (
            ...
            if passed
            else f"Expected blocked status, got '{turn_res.status}'"
        )
        _add("expected_blocked", passed, msg, "reliability")

    if expected.expected_grounding_valid is not None:
        passed = turn_res.grounding_valid is expected.expected_grounding_valid
        _add(
            "expected_grounding_valid",
            passed,
            f"Expected grounding_valid={expected.expected_grounding_valid}, got {turn_res.grounding_valid}",
            "grounding",
        )

    if expected.expected_answerability is not None:
        passed = turn_res.answerability == expected.expected_answerability
        _add(
            "expected_answerability",
            passed,
            f"Expected answerability={expected.expected_answerability}, got {turn_res.answerability}",
            "grounding",
        )

    if expected.expected_grounding_retry_count is not None:
        passed = (
            turn_res.grounding_retry_count == expected.expected_grounding_retry_count
        )
        _add(
            "expected_grounding_retry_count",
            passed,
            f"Expected retry count {expected.expected_grounding_retry_count}, got {turn_res.grounding_retry_count}",
            "grounding",
        )
    ...
    if expected.response_contains_any:
        found_any = any(
            term.lower() in ans_text.lower() for term in expected.response_contains_any
        )
    ...
    if expected.response_contains_all:
        found_all = all(
            term.lower() in ans_text.lower() for term in expected.response_contains_all
        )
    ...
    if expected.response_not_contains:
        found_forbidden = [
            t for t in expected.response_not_contains if t.lower() in ans_text.lower()
        ]
        passed = len(found_forbidden) == 0
